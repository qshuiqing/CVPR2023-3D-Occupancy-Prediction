...
class ConvLSVT(BaseModule):
    def __init__(self,
        ...
        for i in range(n_multi_layer):  # 3
            l_conv = ConvModule(
                in_channels,
                out_channels,
                1,
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        for lvl, mlvl_feat in enumerate(mlvl_feats):  # 3
            ...
            for seq_id in range(len(mlvl_feat_split)):  # 4
                ...
                for batch_id, seq_img_meta in enumerate(img_metas):
                    ...
                    projection = self._compute_projection(img_meta, stride_i).to(feat_i.device)  # (6,3,4)
        ...
        # build outputs
        out = self.fpn_conv(fused_bev)  # (1,64,200,200)
