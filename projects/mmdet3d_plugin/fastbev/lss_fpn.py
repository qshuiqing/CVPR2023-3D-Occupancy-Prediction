...
class FPN_LSS(nn.Module):
    ...
    def forward(self, feats):
        ...
        if self.lateral:  # False
            x2 = self.lateral_conv(x2)
        x1 = self.up(x1)  # (B, C3, H, W)  (1,512,100,100)
        x1 = torch.cat([x2, x1], dim=1)  # (B, C1+C3, H, W) (1,640,100,100)
        x = self.conv(x1)  # (B, C', H, W) (1,512,100,100)
        if self.extra_upsample:  # True
