...
def construct_input(input_shape):
    ego2img = torch.eye(4).float().view(1, 4, 4).expand(24, 4, 4)
    input = dict(img=torch.ones(()).new_empty((1, 24, *input_shape)).cuda(),
                 img_metas=[
                     dict(
                         img_shape=[input_shape] * 24,
                         ego2img=ego2img,
                         origin=np.array([0., 0., 2.2], dtype=np.float32)
                     )
                 ],
                 )
    return input
